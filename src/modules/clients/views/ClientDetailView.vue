...
<template>
  <section class="p-6 md:p-8">
    <RouterLink :to="{ name: 'clients' }" class="text-sm text-indigo-600 hover:underline">‹ Clientes</RouterLink>
    ...
    <template v-else-if="client">
      <header class="mb-6 mt-2">
        ...
        <p class="mt-1 text-sm text-slate-500">
          ...
          <span v-if="client.created_at" class="text-slate-400"> · cliente desde {{ client.created_at }}</span>
        </p>
      </header>
      ...
      <div class="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <article class="rounded-lg border border-slate-200 bg-white p-3">
          ...
          <p class="mt-1 text-xl font-semibold tabular-nums text-slate-800">{{ client.stats.visits }}</p>
        ...
        <article class="rounded-lg border border-slate-200 bg-white p-3">
          ...
          <p class="text-xs text-slate-500">{{ client.stats.favorite_service ?? 'Sin historial' }}</p>
        </article>

        <article
          class="rounded-lg border p-3"
          :class="client.stats.no_shows > 0 ? 'border-red-200 bg-red-50' : 'border-slate-200 bg-white'"
        >
      ...
      <div class="mb-4 flex gap-1 border-b border-slate-200">
        <button
          ...
          :class="tab === option ? 'border-indigo-600 font-medium text-indigo-700' : 'border-transparent text-slate-500'"
          @click="tab = option"
        >
          {{ option }}
          <span v-if="option === 'fotos'" class="text-xs text-slate-400">({{ client.photos.length }})</span>
        </button>
      </div>

      <!-- Historial -->
      <div v-if="tab === 'historial'" class="flex flex-col gap-2">
        <p v-if="!client.history.length" class="rounded-md bg-slate-100 px-4 py-6 text-sm text-slate-600">
      ...
      <div v-else-if="tab === 'fotos'">
        ...
        <p v-if="!client.photos.length" class="rounded-md bg-slate-100 px-4 py-6 text-sm text-slate-600">
        ...
        <div v-else class="grid gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          >
            ...
            <figcaption class="p-2 text-xs">
              <p class="text-slate-700">{{ photo.caption ?? photo.service_name ?? 'Sin descripción' }}</p>
      ...
      <div v-else class="max-w-xl">
        <div class="flex flex-col gap-3">
          ...
          <NxInput v-model="careNotes" label="Antes de atender (alergias, preferencias)" :disabled="saving" />
          <NxInput v-model="notes" label="Notas generales" :disabled="saving" />

          <NxButton v-if="auth.can('clientes.gestionar')" class="self-start" :loading="saving" @click="save">
            Guardar
          </NxButton>
        </div>
      </div>
